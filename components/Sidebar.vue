            ...
            <div id="sidebar-menu">
                ...
                <ul class="metismenu list-unstyled" id="side-menu">
                    <template v-for="item in menuItems">
                        ...
                        <li v-if="!item.isTitle && !item.isLayout" :key="item.id">
                            <a v-if="hasItems(item)" href="javascript:void(0);" class="is-parent" :class="{
                                ...
                                <span :class="`badge badge-pill bg-${item.badge.variant} float-end`
                            ...
                            <nuxt-link :to="item.link" v-if="!hasItems(item)" class="side-nav-link-ref">
                                <i :class="`${item.icon}`" v-if="item.icon"></i>
                                <span>{{ $t(item.label) }}</span>
                                <span :class="`badge badge-pill badge-${item.badge.variant} float-end`
                                    " v-if="item.badge">{{ $t(item.badge.text) }}</span>
                            </nuxt-link>
                    ...
                    <template>
                        <li>
                            <a target="_blank"
                                href="https://drive.google.com/file/d/1wk4c55DDdwQXVz5NVFqYewkV7S_dzLN1/view?usp=sharin"
